  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
        ...
        <div className="w-full lg:w-96 p-6 shrink-0 lg:fixed lg:h-[calc(100vh-4rem)] lg:overflow-y-auto bg-gradient-to-b from-noir-terminal to-background border-r border-noir-border z-10">
            ...
            <h1 className="text-2xl font-bold text-foreground mb-4 line-clamp-2">
                {playlist.title}
            </h1>
            
            <div className="space-y-4 mb-6">
                <div className="text-sm font-medium text-electric-lime">
            ...
            <div className="space-y-2">
                {videos.length === 0 ? (
                    ...
                    videos.map((video, index) => (
                        <Link key={video.id} href={`/watch/${video.id}`} className="group flex gap-4 p-2 rounded-lg hover:bg-noir-terminal border border-transparent hover:border-noir-border transition-all cursor-pointer">
                            <div className="flex items-center justify-center w-8 text-muted-text text-sm font-mono group-hover:hidden">
                                {index + 1}
                            </div>
                            ...
                            <div className="relative w-40 aspect-video bg-noir-bg rounded overflow-hidden shrink-0 border border-noir-border">
                                <Image src={video.thumbnailUrl || "/placeholder.jpg"} alt={video.title} fill className="object-cover" />
                                <div className="absolute bottom-1 right-1 bg-black/80 px-1 py-0.5 text-[10px] text-white font-medium rounded">
