...
import { Search, Globe, Lock, EyeOff, ListVideo } from "lucide-react";
      ...
      try {
        ...
        if (channel) {
          if (activeTab === "Playlists") {
             ...
             const response = await getVideos({ channelId: channel.id, limit: 50, sort: "latest" });
    ...
    <div className="max-w-6xl mx-auto space-y-6">
          ...
          <button
            ...
            className={`pb-3 text-sm font-medium transition-colors border-b-2 ${
              activeTab === tab
                ...
                : "border-transparent text-muted-text hover:text-foreground hover:border-muted-text"
            }`}
          >
            {tab}
          </button>
      ...
      <div className="flex items-center gap-4 py-2">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input 
            placeholder="Filter" 
            className="pl-9 bg-noir-bg border-noir-border focus:border-electric-lime h-9 text-sm"
          />
        </div>
      </div>
      ...
      <div className="border border-noir-border rounded-lg bg-noir-terminal overflow-hidden">
        {/* Header currently shared, or we can conditionalize headers */}
                ...
                playlists.map((playlist) => (
                    <div key={playlist.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-white/5 transition-colors group">
                        <div className="col-span-12 md:col-span-5 flex items-start gap-4">
                            ...
                            <Link href={`/playlist?list=${playlist.id}`} className="w-32 aspect-video bg-noir-bg border border-noir-border rounded overflow-hidden shrink-0 relative group-hover:border-electric-lime transition-colors block">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-xs text-muted-text mt-1 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                                    <button className="hover:text-electric-lime">Edit</button>
                                    <span>•</span>
                                    <button className="hover:text-electric-lime">Videos</button>
                                </p>
                            </div>
                        </div>
             ...
             videos.map((video) => (
                ...
                <div className="col-span-12 md:col-span-5 flex items-start gap-4">
                    ...
                    <p className="text-xs text-muted-text mt-1 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                        <button className="hover:text-electric-lime">Edit</button>
                        <span>•</span>
                        <button className="hover:text-electric-lime">Analytics</button>
                        <span>•</span>
                        <button className="hover:text-electric-lime">Comments</button>
                    </p>
                    </div>
                </div>
                ...
                <div className="col-span-1 text-right text-sm text-foreground hidden md:block">--</div>
                <div className="col-span-1 text-right text-sm text-foreground hidden md:block">{video.likes || "--"}</div>
                </div>
            ))
          )}
        </div>
