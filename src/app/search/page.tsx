                ...
                {results.channels.map((channel) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-foreground truncate group-hover:text-electric-lime transition-colors">
                        {channel.name}
                      </h3>
                      <p className="text-sm text-muted-text">@{channel.handle}</p>
                      <p className="text-xs text-muted-text">
                        {Intl.NumberFormat("en-US", { notation: "compact" }).format(channel.subscriberCount)} subscribers
                      </p>
                    </div>
                  </Link>
                ))}
              ...
              <div className="space-y-4">
                {results.videos.map((video) => (
                  <Link
                    key={video.id}
                    href={`/watch/${video.id}`}
                    className="flex gap-4 p-2 -m-2 rounded-lg hover:bg-noir-terminal/50 transition-colors group"
                  >
                    {/* Thumbnail */}
                    <div className="relative w-64 min-w-[16rem] aspect-video rounded-lg overflow-hidden bg-noir-terminal">
                      <Image
                    ...
                    <div className="flex flex-col gap-2 py-1">
                      <h3 className="font-semibold text-foreground line-clamp-2 group-hover:text-electric-lime transition-colors">
                        {video.title}
                      </h3>
