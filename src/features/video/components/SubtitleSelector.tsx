 ...
 */
export function SubtitleSelector({ tracks, status, activeTrack, onTrackSelect, className, container }: SubtitleSelectorProps) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className={cn("text-white hover:bg-white/10 h-10 w-10 relative cursor-pointer", activeTrack && "text-electric-lime", className)}>
          {isProcessing ? <Loader2 className="h-5 w-5 animate-spin" /> : <Languages className="h-5 w-5" />}
          {activeTrack && <span className="absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full bg-electric-lime" />}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="bg-noir-terminal border-white/10 text-white min-w-[160px]" container={container}>
        ...
        {/* Off option */}
        <DropdownMenuItem
          className={cn("flex items-center justify-between gap-4 focus:bg-white/10 focus:text-white cursor-pointer", !activeTrack && "text-electric-lime")}
