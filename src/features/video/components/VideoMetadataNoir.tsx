  ...
  return (
    <div className="mt-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b border-noir-border">
        ...
        <div className="flex items-center gap-4">
          <Avatar className="h-12 w-12 border border-noir-border grayscale">
            ...
            <AvatarFallback className="bg-noir-terminal text-electric-lime font-bold">{creator.username[0]}</AvatarFallback>
          </Avatar>
          <div className="flex flex-col">
            <span className="text-sm font-bold text-electric-lime">{creator.username}</span>
            <span className="text-xs text-muted-text">{creator.subscribers} Subscribers</span>
          </div>
          <Button
            variant="outline"
            size="sm"
      ...
      <div className="flex items-center gap-4 text-[10px] font-mono uppercase tracking-[0.15em] text-muted-text">
        ...
        <span>
          VWS: <span className="text-white">{formattedViews}</span>
        </span>
        <span className="text-noir-border">|</span>
        <span>
          UPLOADED: <span className="text-white">{uploadedAt.toUpperCase().replace(/ /g, "_")}</span>
        </span>
      </div>

      {/* Description */}
      <div className="bg-noir-terminal/50 border border-noir-border p-4 space-y-3">
        <pre className={`text-xs font-mono text-white/80 whitespace-pre-wrap leading-relaxed ${!isDescriptionExpanded ? "line-clamp-3" : ""}`}>{description}</pre>
        <button onClick={() => setIsDescriptionExpanded(!isDescriptionExpanded)} className="flex items-center gap-1 text-[10px] font-mono uppercase tracking-widest text-electric-lime hover:underline">
