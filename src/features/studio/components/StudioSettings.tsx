...
export function StudioSettings({ settings, onChange, disabled, className }: StudioSettingsProps) {
  ...
  return (
    <div className={`bg-noir-terminal border border-noir-border rounded-xl p-6 space-y-6 ${disabled ? "opacity-50 pointer-events-none" : ""} ${className}`}>
      {/* Title */}
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            >
              ...
              <span className={`text-sm font-medium block ${settings.visibility === option.id ? "text-foreground" : "text-muted-text"}`}>
                {option.label}
              </span>
