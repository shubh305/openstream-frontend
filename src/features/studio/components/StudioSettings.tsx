    ...
    <div className={`bg-noir-terminal border border-noir-border rounded-xl p-4 sm:p-6 !space-y-6 ${disabled ? "opacity-50 pointer-events-none" : ""} ${className}`}>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          {/* Title */}
          <div>
            <label className="text-xs text-muted-text uppercase tracking-wide mb-2 block font-black">
            ...
            <Input
              ...
              className="bg-noir-bg border-noir-border focus:border-electric-lime h-12 text-sm rounded-lg"
            />
          </div>

          {/* Category */}
          <div>
            <label className="text-xs text-muted-text uppercase tracking-wide mb-2 block font-black">Category</label>
            <select
          ...
          {/* Description */}
          <div>
            <label className="text-xs text-muted-text uppercase tracking-wide mb-2 block font-black">Stream Description</label>
            <textarea
        ...
        <div className="space-y-6">
          {/* Visibility */}
          <div>
            <label className="text-xs text-muted-text uppercase tracking-wide mb-3 block font-black">Visibility</label>
            <div className="grid grid-cols-1 gap-3">
              {[
                ...
                >
                  <option.icon className={`w-5 h-5 ${settings.visibility === option.id ? "text-electric-lime" : "text-muted-text"}`} />
                  <div>
                    <span className={`text-sm font-black block uppercase tracking-wider ${settings.visibility === option.id ? "text-foreground" : "text-muted-text"}`}>{option.label}</span>
                    <span className="text-[10px] text-muted-text uppercase font-bold tracking-widest">{option.desc}</span>
                  </div>
                </button>
              ))}
            </div>
