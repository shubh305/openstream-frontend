...
import { Home, Search, Tv, Sparkles, X, User as UserIcon, Settings, LogOut, Bell, Video, BarChart2 } from "lucide-react";
...
export function BottomNavbar({ user }: BottomNavbarProps) {
  ...
  return (
    <>
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 z-[160] bg-black flex flex-col pt-safe px-4">
          <div className="flex items-center gap-4 py-6">
            ...
            <button onClick={() => setIsSearchOpen(false)} className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-white cursor-pointer">
              <X className="w-6 h-6" />
            </button>
          </div>
      ...
      {isAccountOpen && (
        <div className="fixed inset-0 z-[150] bg-black overflow-hidden flex flex-col">
              ...
              <div className="flex flex-col items-center text-center space-y-6 pt-10">
                ...
                <div className="flex flex-col w-full gap-3 px-4">
                  <Link
                    ...
                    className="w-full py-4 rounded-2xl bg-white text-black font-black text-xs uppercase tracking-[0.2em] text-center cursor-pointer"
                  >
                    Login
                  </Link>
                  <Link
                    href="/signup"
                    onClick={() => setIsAccountOpen(false)}
                    className="w-full py-4 rounded-2xl bg-white/5 text-white font-black text-xs uppercase tracking-[0.2em] text-center border border-white/10 cursor-pointer"
                  >
                    Sign Up
                  </Link>
                </div>
            ...
            {user && (
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <button onClick={handleLogout} className="flex items-center justify-between p-5 rounded-3xl glasswork border border-signal-red/10 active:bg-signal-red/10 group mt-4 cursor-pointer">
                    <div className="flex items-center gap-4">
          ...
          <button onClick={() => setIsAccountOpen(false)} className="absolute top-6 right-6 w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-white z-[170] cursor-pointer">
            <X className="w-6 h-6" />
          </button>
        </div>
      )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[200] md:hidden">
        ...
        <div className="mx-2 mb-4 h-16 glasswork glass-border border-white/10 rounded-2xl flex items-center justify-around px-2 shadow-2xl shadow-black/50 relative">
          {NAV_ITEMS.map((item, idx) => {
            ...
            const content = (
              <div className={cn("p-2 rounded-xl transition-all duration-300 flex flex-col items-center", isActive ? "bg-white/10 text-white translate-y-[-4px]" : "text-muted-text hover:text-white")}>
                {item.isAvatar && user ? (
                  <div className={cn("w-7 h-7 rounded-sm overflow-hidden border border-white/10 transition-all", isActive && "border-white ring-2 ring-white/20")}>
                    <Image src={user.avatarUrl || ""} alt={user.username} width={28} height={28} className="object-cover h-full grayscale" />
                  </div>
                ) : Icon ? (
                  <Icon className={cn("w-6 h-6", isActive && "drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]")} />
                ) : null}
            ...
            if (item.href) {
              return (
                <Link key={idx} href={item.href} onClick={item.onClick} className="relative flex flex-col items-center justify-center py-1 transition-all duration-300 active:scale-90 cursor-pointer">
                  {content}
                  {isActive && <span className="absolute bottom-1 w-1 h-1 rounded-full bg-electric-lime shadow-[0_0_8px_rgba(180,255,0,0.8)]" />}
                </Link>
              );
            }

            return (
              <button key={idx} onClick={item.onClick} className="relative flex flex-col items-center justify-center py-1 transition-all duration-300 active:scale-90 cursor-pointer">
                {content}
                {isActive && <span className="absolute bottom-1 w-1 h-1 rounded-full bg-electric-lime shadow-[0_0_8px_rgba(180,255,0,0.8)]" />}
              </button>
            );
